...
import { calculateReadiness, formatDate, scoreColor, progressBarColor } from '@/lib/utils'

/** Dashboard: lists all releases with readiness score, go/no-go status, and progress bar. */
export default async function HomePage() {
  const releases = await prisma.release.findMany({
    include: { items: true, features: true },
      ...
      {releases.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-xl border border-gray-200">
          ...
          <p className="text-sm text-gray-500 mb-6">
            Create your first release to start tracking readiness.
          </p>
          <Link
            href="/releases/new"
        ...
        <div className="space-y-3">
          {releases.map((release) => {
            const { score, isGo, completedTotal, totalItems } = calculateReadiness(release.items, {
                ...
                <div className="mt-4">
                  <div className="flex items-center gap-2 mb-1.5">
                    ...
                    <span className="text-xs text-gray-500 tabular-nums shrink-0">
                      {completedTotal}/{totalItems} items
                    </span>
                  </div>
                </div>
              </Link>
